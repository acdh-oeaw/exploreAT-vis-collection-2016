...
var Visualizer = {
    ...
    signalWordFound : function (w) {
        ...
        $(w).css("background",'#298cbb').css("color","white").animate({"opacity": 'hide'},1000,"linear",
        function () {
            $(w).css("background",'white').css("color","#ddd");
...
function getRandomNames(){
    ...
    esClient.search({
        ...
        headers: {
            ...
            var commonObjects = resp.hits.hits;
            shuffle(commonObjects);

            for(var i=0; i<commonObjects.length && i<6; i++){
                if(commonObjects[i]._source.commonName.indexOf(".") == -1 &&
                commonObjects[i]._source.commonName.indexOf(" ") == -1 &&
                commonObjects[i]._source.commonName.indexOf("\'") == -1 &&
                commonObjects[i]._source.commonName.length < charLimit){
                    commonNames += ""+commonObjects[i]._source.commonName;
                    if(i<(6-1)) commonNames += ",";
                }
            }
    ...
    function generateSoup(){
        ...
        setTimeout(function(){
            $("#spinner").remove();
            $("#theGrid").wordsearchwidget({"wordlist" : commonNames,"gridsize" : charLimit+2});
        },1500)
    }
