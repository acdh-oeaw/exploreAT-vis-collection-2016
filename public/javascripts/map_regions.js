...
queue()
    ...
    .defer(d3.json, "./data/places_out.json")
        .defer(d3.json, "/region?target=503")
    //.defer(d3.json, "/all_regions")///region?target=501")
    .await(makeMap);
...
//target = {"coordinates": [[[[3.78, 9.28], [-130.91, 1.52], [35.12, 72.234], [3.78, 9.28]]], [[[23.18, -34.29], [-1.31, -4.61], [3.41, 77.91], [23.18, -34.29]]]], "type": "MultiPolygon"};
  ...
  svg.append("path")
      .datum(topojson.mesh(exploreAT, exploreAT.objects.subunits, function(a, b) { return a !== b && a.id !== "DEU"; }))
      .attr("d", path)
      .attr("class", "subunit-boundary");

  svg.append("path")
      .datum(topojson.mesh(exploreAT, exploreAT.objects.subunits, function(a, b) { return a === b && a.id === "AUT"; }))
      .attr("d", path)
  ...
  svg.selectAll(".place-label AT")
    ...
    refreshIntervalId = setInterval(function () {
      ...
      function update(id){
          ...
          {
          queue()
            .defer(d3.json, "/region?target="+id)
            .await(newTarget);
            }
           else {
...
*/
      ...
      function tipText(d){
           ...
           tip.html(d.properties.SOV0NAME + "<br><strong style='color:SteelBlue'>Name: </strong><span>" + d.properties.NAME + "</span> ("
                 + d.properties.ADM1NAME + ")<br/><strong style='color:SteelBlue'> pop: </strong><span>" + d.properties.POP_MAX + "</span>");
           tip.show();
      }
